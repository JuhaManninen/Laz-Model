...
type
  ...
  private
    ...
    FSourcePos: TPoint;
    FTokenPos: TPoint;
    FNextTokenPos: TPoint;

    FStream: TMemoryStream;
    FActualCurrPos: PChar;
    FToken: string;
    FNextToken: string;
    ...
    FMarkSourcePos: TPoint;
    FMarkTokenPos: TPoint;
    FMarkNextTokenPos: TPoint;
...
//    procedure UndentAfter;
    ...
    procedure StepCurrPos;

    procedure Mark;
    procedure Recall;

    property  FCurrPos: PChar read FActualCurrPos;
  protected
  ...
  public
    ...
    property Filename: String read FFullName write FFullName;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          FStream.Position := 0;
          { TODO : File position will be wrong when we have include files but... }
          FActualCurrPos := PCHar(integer(FStream.Memory) + Offset);
        finally
          IncStr.Free;
          Buf.Free;
        end;
...
begin
  inComment := False;
  continueLastComment := False;
  isNewLine := (FCurrPos = FStream.Memory);
  State := True;
  while State do
  begin
    State := False;
    ...
    begin
      continueLastComment := False;
      if not isNewLine then FComment := '';
      isNewLine := True;
    end;
...
begin
  FToken := FNextToken;
  FTokenPos := FNextTokenPos;
  FNextToken := '';
  EatWhiteSpace;
  FNextTokenPos := FSourcePos;
  case FCurrPos^ of
    #0 : // End of File
      begin
        if FToken <> '' then
...
procedure TDelphiParser.ParsePackage;
begin
  ...
  begin
    ...
    FComment := '';
    SkipToken(';');
    if lToken = 'requires' then
    begin
      // Skip the requires clause
      while GetNextToken and (lToken <> ';') do;
      if lToken = ';' then GetNextToken;
    end;
    if lToken = 'contains' then
      ParseUses;
  end;
end;
...
begin // Token = Attributename
  ...
  try
    ...
    begin
      GetNextToken; GetNextToken;
      attrib := (AAttribute.Owner as TClass).AddAttribute(Token);
      attrib.Documentation.Description := FComment;
      FComment := '';
      attribs.Add(attrib);
    end;
    if NextToken = ':' then
...
//  Result := nil;
  ...
  begin
    ...
    if (lToken='function') or (lToken='procedure') then
    begin
      name := Token;
      ParseFunction(nil);
      break;
...
begin
  ...
  FMarkSourcePos := FSourcePos;
  FMarkTokenPos  := FTokenPos;
  FMarkNextTokenPos  := FNextTokenPos;
end;

procedure TDelphiParser.Recall;
begin
  FActualCurrPos   := FMarkCurrPos;
  FToken     := FMarkToken;
  FNextToken := FMarkNextToken;
  FSourcePos := FMarkSourcePos;
  FTokenPos  := FMarkTokenPos;
  FNextTokenPos  := FMarkNextTokenPos;
end;

procedure TDelphiParser.StepCurrPos;
begin
  inc(FActualCurrPos);

  if FActualCurrPos^ = #10 then
  begin
    inc(FSourcePos.Y);
    FSourcePos.X := 0;
  end else
